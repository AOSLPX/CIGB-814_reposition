...
try:
    ...
    try:
        import esm
        load_model_and_alphabet = esm.pretrained.load_model_and_alphabet
    except (ImportError, AttributeError):
        try:
            import esm.pretrained as pretrained
            load_model_and_alphabet = pretrained.load_model_and_alphabet
        except (ImportError, AttributeError):
            raise ImportError(
                "无法导入 fair-esm。请运行以下命令安装：\n"
...
class ESM(nn.Module):
    def __init__(self, config):
        ...
        # 加载ESM2模型（使用模型名称字符串）
        self.esm_model, self.alphabet = load_model_and_alphabet("esm2_t33_650M_UR50D")
